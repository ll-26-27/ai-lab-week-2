...
Claude through OpenRouter (the tutorial's first generation), verified 2026-09-17 with "Reply with the single word HELLO.":

```bash
node utils/generate-text.mjs openrouter "Reply with the single word HELLO." --model anthropic/claude-sonnet-5
```
```bash
node utils/generate-text.mjs openrouter "Reply with the single word HELLO." --model anthropic/claude-haiku-4.5
```
```bash
node utils/generate-text.mjs openrouter "Reply with the single word HELLO." --model anthropic/claude-opus-5
```

All three worked. Cost reported: $0.000096 (Sonnet), $0.00004 (Haiku), $0.00024 (Opus). `node utils/list-text-models.mjs --grep anthropic` prints the current Claude ids.
...
| openrouter | text (Claude) | yes | anthropic/claude-sonnet-5, anthropic/claude-haiku-4.5, anthropic/claude-opus-5 | $0.00004 to $0.00024 |
