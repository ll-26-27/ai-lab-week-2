...
Providers: `openrouter` (`OPENROUTER_API_KEY`), `fal` (`FAL_API_KEY`), `huit-openai` / `huit-gemini` / `huit-bedrock` (`HUIT_API_KEY`). Reference images: `--ref FILE` (PNG, JPEG, or WebP; repeatable) goes to OpenRouter as `input_references`, to fal as `image_urls` (or one image in `image_url` with `--ref-field image_url`), and to HUIT Gemini inline; on fal pick the editing endpoint (`.../edit`, `.../kontext`). How many each model accepts is in the glossary entries on the OpenRouter and fal image APIs. Video endpoints are not wired up. `--help` on any script lists its options; `--dry-run` shows the request or the ffmpeg commands without sending or running anything. `examples.md` is a sheet of commands verified against every route, with the models and dates that worked.
