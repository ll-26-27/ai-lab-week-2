...
// and writes player.html into that folder by default. --out picks another location; the media path in the page is
...
try {
  ...
  const model = JSON.parse(readFileSync(join(resolve(responseFile, '..'), 'request.json'), 'utf8').toString?.() ? readFileSync(join(resolve(responseFile, '..'), 'request.json'), 'utf8') : '{}').model;
  const html = playerHtml({title: o.title || basename(media), mediaHref: mediaHref(out, media), video: isVideo(media), segments, model});
